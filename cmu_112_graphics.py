...
MAJOR_VERSION = 0
MINOR_VERSION = 9.5
LAST_UPDATED = datetime.date(year=2020, month=8, day=4)
...
class App(object):
    ...
    @_safeMethod
    def _mouseMotionWrapper(self):
        if (not self._running): return
        mouseMovedExists = self._methodIsOverridden('mouseMoved')
        mouseDraggedExists = self._methodIsOverridden('mouseDragged')
        if ((not self._paused) and
            (not self._mousePressedOutsideWindow) and
            (((not self._mouseIsPressed) and mouseMovedExists) or
             (self._mouseIsPressed and mouseDraggedExists))):
            ...
            if ((self._lastMousePosn !=  (event.x, event.y)) and
                (event.x >= 0) and (event.x <= self.width) and
                (event.y >= 0) and (event.y <= self.height)):
                if (self._mouseIsPressed): self.mouseDragged(event)
                else: self.mouseMoved(event)
                self._lastMousePosn = (event.x, event.y)
                self._redrawAllWrapper()
        if (mouseMovedExists or mouseDraggedExists):
            self._deferredMethodCall(afterId='mouseMotionWrapper', afterDelay=self.mouseMovedDelay, afterFn=self._mouseMotionWrapper)
